...
export class Event {
    ...
    public between(start: Date, end: Date): EventInstance[] {
        ...
        const instances = this.rrules.between(new Date(start.getTime() - this.end.difference - 24 * 60 * 60 * 1000), new Date(end.getTime() + 24 * 60 * 60 * 1000), true);
    ...
    data[2].forEach((component) => {
        if (component[0] === 'vevent') {
            if (component[1].find((property) => property[0] === 'summary') === undefined) {
                component[1].push(ICALJS.parse.property('SUMMARY:'));
            }
        }
    })

    return ICALJS.stringify(data);
}